...
Also provides an implementation of a search for APN permutations over the Galois ring $\mathrm{GR}(4,2) \cong \mathbb{Z}_4 \times  \mathbb{Z}_4$. Reproduce the search, run the files in the following order
- GR42_APN_exhaustive.py: Exhaustively searches through all permutations of $\mathrm{GR}(4,2)$ with three specified fixed points. Produces the files "all-42-apns.npy" and "all-42-apns.npy", which contain the resulting functions, stored as tables. 
